...
function displayCurrentQuestion() {
    ...
    optionButtons.forEach((button, index) => {
        const stationFullName = question.options[index];
        const parts = stationFullName.split('（');
        const stationName = parts[0];
        const stationReading = parts.length > 1 ? '（' + parts[1] : '';
        
        // ボタン内を2行で表示するためのHTML構造
        button.innerHTML = `<span class="station-name">${stationName}</span><span class="station-reading">${stationReading}</span>`;
...
function handleAnswer(selectedIndex) {
    ...
    // 駅名部分のみを取得（括弧を除く）
    const correctStationName = question.correctAnswer.split('（')[0];
    ...
    if (isCorrect) {
        const praises = [
            `大正解！「${correctStationName}」駅です！🎉`,
            `さすが！「${correctStationName}」駅が正解です！✨`,
            `素晴らしい！「${correctStationName}」駅です！👍`,
            `完璧！「${correctStationName}」駅ですね！🌟`
        ];
        ...
        const encouragements = [
            `残念！正解は「${correctStationName}」駅です。次は当てよう！`,
            `惜しい！正解は「${correctStationName}」駅でした。次頑張って！`,
            `ざんねん！正解は「${correctStationName}」駅です。次回に期待！`,
            `おしい！正解は「${correctStationName}」駅でした。次問に挑戦！`
        ];
